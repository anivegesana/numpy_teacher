...
def dataset_loader(mode="amazing_python", split="train", proportion=1.0, n_files=float('inf')) -> List[List[str]]:
    if mode == "amazing_python":
        ...
        final_dataset = collect_python_files(PY_150_PATH, n_files)
        upper_bound = int(len(final_dataset) * min(1.0, proportion))
        return final_dataset[:upper_bound]
...
def split_by_import(flat_dataset: List[str], n_files=float('inf')) -> List[List[str]]:
    import_seen = False
    ...
    for row in flat_dataset:
        ...
        if len(dataset) > n_files:
            return dataset

    return dataset


def collect_python_files(python_repositories_dir: str, n_files=float('inf')) -> List[List[str]]:
    # print(os.getcwd())
    dataset = []
